...
class Q_EXPORT LoginRequest : public Request
{
public:
    LoginRequest();
protected:
...
class Q_EXPORT PublicTimelineRequest : public Request
{
public:
...
class Q_EXPORT HomeTimelineRequest : public Request
{
public:
    HomeTimelineRequest();
protected:
    void initParameters() {
        qDebug("HomeTimelineRequest::initParameters");
        (*this)
        ("source", "") //false 	string 	采用OAuth授权方式不需要此参数，其他授权方式为必填参数，数值为应用的AppKey。
        ("access_token", "")// 	false 	string 	采用OAuth授权方式为必填参数，其他授权方式不需要此参数，OAuth授权后获得。
...
//获取用户发布的微博
class Q_EXPORT UserTimelineRequest : public Request
{
public:
    UserTimelineRequest();
protected:
...
//获取用户发布的微博的ID
class Q_EXPORT UserTimelineIdsRequest : public Request
{
public:
    UserTimelineIdsRequest();
protected:
    /*
     *    参数uid与screen_name二者必选其一，且只能选其一
     *    参数uid与screen_name都没有或错误，则默认返回当前登录用户的数据
